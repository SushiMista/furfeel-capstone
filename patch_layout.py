...
# 1. Replace _buildStepContainer
old_step_container_pattern = re.compile(r'  Widget _buildStepContainer\(\{.*?return SafeArea\([\s\S]*?child: _busy[\s\S]*?\? const SizedBox\(height: 24, width: 24, child: CircularProgressIndicator\(strokeWidth: 2, color: Colors\.white\)\)[\s\S]*?: const Text\(\'Continue\'\),[\s\S]*?\),[\s\S]*?\);[\s\S]*?\}', re.DOTALL)
...
content = old_step_container_pattern.sub(new_step_container, content)

# 2. Replace build method to remove old progress bar
old_build_pattern = re.compile(r'  Widget build\(BuildContext context\) \{.*?_buildCompletion\(\),\n              \],\n            \),[\s\S]*?\.animate\(\)\.fadeIn\(\),\n          \],\n        \),\n      \),\n    \);', re.DOTALL)
...
content = old_build_pattern.sub(new_build, content)
