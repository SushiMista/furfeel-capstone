...
function dart() {
  ...
  lines.push("// ADDED: dark theme — colors resolve through FurFeelTokens.isDark, which the");
  lines.push("// app root sets from user_settings.theme (+ platform brightness) before each");
  lines.push("// build. The full-tree rebuild on theme change keeps every widget in sync.");
  lines.push("// ponytail: static flag + getters over a ThemeExtension refactor; migrate if");
  lines.push("// per-subtree theming is ever needed.");
  lines.push("class _Palette {");
  lines.push("  const _Palette({");
  ...
  const paletteLiteral = (entries) =>
    ["_Palette(", ...entries.map(([k, v]) => `  ${k}: ${dartColor(v)},`), ")"].join("\n");
  lines.push(`const _light = ${paletteLiteral(light)};`);
  lines.push("");
  lines.push(`const _dark = ${paletteLiteral(dark)};`);
  lines.push("");
  lines.push("/// FurFeel design tokens (docs/19 Design Guide — blue + white).");
  lines.push("abstract final class FurFeelTokens {");
  lines.push("  /// Set by the app root before building; see furfeel_theme.dart.");
  lines.push("  static bool isDark = false;");
  lines.push("  static _Palette get _p => isDark ? _dark : _light;");
  lines.push("");
  lines.push("  // Color — resolved against the active theme (word + dot always, never color alone)");
  for (const [k] of light) lines.push(`  static Color get ${k} => _p.${k};`);
  lines.push("");
