...
## ADR-021: Care Reminders Are Local Notifications, Not Server Push
Status: Accepted (2026-07-29)

Decision: Owner care reminders (P3.12 — medication, feeding, vet visits) fire as on-device scheduled notifications via `flutter_local_notifications`, not through a server/Edge push path. New `care_reminders` table (`dog_id`, `user_id`, `title`, `notes`, `due_at`, `repeat` ∈ none/daily/weekly, `active`), owner-only RLS gated on `dogs.owner_user_id = auth.uid()` — the same shape as `stress_labels_select_owner`, no clinic access, no widening of existing policies. The mobile `ReminderScheduler` reschedules per reminder (stable notification id derived from the reminder uuid, so it never `cancelAll`s and clobbers a sibling dog's reminders) and honors the existing `user_settings` master notification toggle + quiet hours (`effectiveFireTime` defers a reminder landing inside quiet hours to the window's end).

Reason: FCM/APNs is still unwired (`push_registration.dart` is a deliberate no-op until platform credentials exist), so a push-based reminder would ship dead. Local notifications need no server, no device token, and no Firebase project — the feature works today and stays self-contained. Scheduling is `inexactAllowWhileIdle`, which sidesteps Android 12+'s `SCHEDULE_EXACT_ALARM` permission entirely; a reminder is a soft nudge, not an alarm clock, so a few minutes of OS batching is acceptable. Reminders are owner-authored free text, never a classifier input and never diagnosis output, so no guardrail surface is touched.

Consequence / follow-up: reminders live only on the device that scheduled them — deleting a reminder on another device won't cancel its already-scheduled local notification there until that device reloads. Acceptable for MVP; a server-push path (once FCM lands) would centralize this. iOS foreground-presentation delegate and a real device-run verification of delivery are flagged as human follow-up.
