...
### care_guidance
Vet-authored guidance shown by the owner app's Care Insights (informational only, never diagnosis).
| column | type | constraints |
|---|---|---|
| id | uuid | PK |
| stress_level | stress_level | not null |
...
RLS: readable by any authenticated user for their dog's clinic (or global); writable by vets/admin.
