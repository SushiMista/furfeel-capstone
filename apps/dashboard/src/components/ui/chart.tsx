...
export function ChartTooltipContent({
  ...
  return (
    <div className="rounded-lg border border-hairline bg-surface p-2 shadow-md text-xs">
      {!hideLabel && <div className="font-semibold text-ink mb-1">{label}</div>}
      <div className="flex flex-col gap-1">
        {payload.map((item: any, index: number) => {
          return (
            <div key={index} className="flex items-center gap-1.5 text-ink-muted">
              <span
