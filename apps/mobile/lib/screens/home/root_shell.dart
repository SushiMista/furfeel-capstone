...
import 'package:furfeel_mobile/data/push_registration.dart';
...
import 'package:furfeel_mobile/util/perf.dart';
...
class _RootShellState extends State<RootShell> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      // The Home tab owns its top: the immersive status hero IS the header
      // (docs/22 v7), so the shell app bar is dropped there to avoid a double
      // "FurFeel" header. Other tabs keep the brand logo + dog switcher.
      appBar: _tab == 0
          ? null
          : AppBar(
              title: const FurFeelLogo(),
              actions: [
                // The switcher acts on the one selected dog; Chat picks its own.
                if (dogs != null && dog != null && _tab != 4)
                  Padding(
